...
正式 Android 应用已经支持在不重新安装 APK 的情况下更新饮食助手和安心科普。应用以离线内置内容兜底，从 GitHub Pages（失败时使用仓库静态文件）读取版本清单，只接收通过结构校验和人工审核的公开内容。个人健康资料始终留在手机本地。

“我的 → 饮食与科普内容更新”提供当前版本、食物和文章数量、发布日期、上次检查时间及手动检查按钮。同步失败不会覆盖旧内容，也不会改变用药和复查提醒。
